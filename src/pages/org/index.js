...
const Org = (props) => {
    ...
    return (
        <Wrapper>
            <div className="Org">
                <img className="OrgBanner" src={`${process.env.PUBLIC_URL}/img/organisation/banner1.png`} />
                <span className="OrgBannerTitle">{showdata.title}</span>
                <span className="OrgTextOne" dangerouslySetInnerHTML={{ __html: showdata.titlebannertext }}></span>
                <span className="OrgTextBtn" style={{ width: homepage.chiFlag === 'chi' ? '20rem' : '30rem' }}><span>{showdata.titleBtntext}</span></span>
