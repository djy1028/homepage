...
import { Button, Descriptions, Divider, Input, message, Space, Spin, Steps, Upload } from 'antd';
import {CheckCircleTwoTone,CloseCircleTwoTone,CloudUploadOutlined}  from '@ant-design/icons'
import { downloadApplication, getToken } from 'auth-provider';
...
import { useStuproupload } from 'utils/student';
...
export const Detail = ()=>{
    ...
    return (
        ...
        <NewStep direction={'vertical'} 
            current={applyInfo && applyInfo.status<=6?0: 
                applyInfo && applyInfo.status<11?1:
                applyInfo && applyInfo.status === 11?2:
                applyInfo && applyInfo.status<=15?3:4}>
            <Steps.Step title={<strong>{t('admin.firsttrial.step_title.0')}</strong>} description={
                <>
                    <Des column={2}>
                        ...
                        {applyInfo.userName && <Descriptions.Item label={t('admin.firsttrial.first.0')}>{applyInfo.userName}</Descriptions.Item>}
            ...
            <Steps.Step title={<strong>{t('admin.firsttrial.step_title.1')}</strong>} description={
                    ...
                    <>
                        <Des column={2}>
                            ...
                            {applyInfo.userName && <Descriptions.Item label={t('admin.firsttrial.mid.0')}>{applyInfo.userName}</Descriptions.Item>}
            ...
            <Steps.Step title={<strong>{t('admin.firsttrial.step_title.2')}</strong>} description={
                  <Des column={1}>
                        {applyInfo && applyInfo.status>=11 && <Descriptions.Item label={''}>{t('admin.firsttrial.bonus')}</Descriptions.Item>}
                  </Des>
            } />
            ...
            <Steps.Step title={<strong>{t('admin.firsttrial.step_title.3')}</strong>} description={
                 ...
                 <>
                     <Des column={2}>
                         ...
                         {applyInfo.userName && <Descriptions.Item label={t('admin.firsttrial.end.0')}>{applyInfo.userName}</Descriptions.Item>}
